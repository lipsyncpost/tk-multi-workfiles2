...
class InteractiveOpenAction(OpenFileAction):
    ...
    def _open_workfile_with_check(
        self, work_file, publish_file, env, next_version, parent_ui
    ):
        """
        ...
        if publish_file and work_file.compare_with_publish(publish_file) < 0:
            ...
            form = OpenOptionsForm(
                None,
                self._app,
                work_file,
                publish_file,
                OpenOptionsForm.OPEN_WORKFILE_MODE,
                next_version,
                publish_requires_copy,
            )
            open_mode = WrapperDialog.show_modal(
                form, parent_ui, "Found a More Recent Publish!"
            )
        ...
        if open_mode == OpenOptionsForm.OPEN_WORKFILE:
            ...
            if not self._open_publish_as_workfile(
                publish_file, env, next_version, parent_ui
            ):
                return False
        else:
            # cancelled so stop!
    ...
    def _open_publish_with_check(
        self, publish_file, work_file, env, next_version, parent_ui
    ):
        """
        Function called when user clicks Open for a file
        in the Publish Area
        ...
        if work_file or publish_requires_copy:
            ...
            mode = (
                OpenOptionsForm.OPEN_PUBLISH_MODE
                if publish_requires_copy
                else OpenOptionsForm.OPEN_PUBLISH_NO_READONLY_MODE
            )
            form = OpenOptionsForm(
                None,
                self._app,
            ...
            if open_mode == OpenOptionsForm.OPEN_WORKFILE:
                ...
                # open the published file instead:
                return self._open_publish_as_workfile(
                    publish_file, env, next_version, parent_ui
                )
            ...
            if current_user and current_user["id"] != env.context.user["id"]:
                ...
                try:
                    ...
                    self._app.log_exception(
                        "Failed to resolve user sandbox file path %s" % work_path
                    )
                    return False
    ...
    def _open_publish_as_workfile(self, file, env, new_version, parent_ui):
        ...
        if (
            ...
            and "version" not in env.publish_template.keys
        ):
            ...
            try:
                ...
                if (
                    current_user
                    and sp_ctx.user
                    and sp_ctx.user["id"] != current_user["id"]
                ):
                ...
                self._app.log_exception(
                    "Failed to resolve work file path from publish path: %s" % src_path
                )
                return False
